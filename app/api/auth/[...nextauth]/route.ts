  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        // Fallback default if decoding failed or exp not present
        if (!token.accessTokenExpires) {
          // If the backend doesn't provide expires_in or exp claim, 
          // default to 15 minutes to be safe (refreshes more often)
          const expiresIn = (user as any).expiresIn || 900 // 15 mins
          token.accessTokenExpires = Date.now() + expiresIn * 1000
        }
      }

      // If the token has not expired yet (with a 1-minute safety buffer), return it
      const safetyBuffer = 60 * 1000
      if (token.accessToken && Date.now() < (token.accessTokenExpires as number) - safetyBuffer) {
        return token
      }

      // If the token has expired or is about to expire, try to refresh it
      try {
        ...
        return {
          ...
          accessTokenExpires: newExpiry || (Date.now() + 900 * 1000), // Default to 15 mins fallback
        }
      } catch (error) {
